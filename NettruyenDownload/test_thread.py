...
def download_comic_total(url, path):
    def get_data(areas):
        ...
        for area in areas:
            ...
            for chap_link in chap_links:
                chap_response = session.get(chap_link)
    ...
    def download_chapter(chap_imgs, chap_path):
        for img_count, img_link in enumerate(chap_imgs, start=1):
            ...
            response_img = session.get(img_link_fix)
    ...
    def download_comic(data, comic_name):
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for index, record in enumerate(data, start=1):
                ...
                future = executor.submit(download_chapter, chap_imgs, chap_path)
                futures.append(future)

            for index, future in enumerate(futures, start=1):
                future.result()
                progress_end(index, data[index-1]['chapter_name'])
